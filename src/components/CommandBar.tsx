...
type Mode = "tour" | "explorer" | "researcher";
...
export const CommandBar = ({ currentMode, onModeChange }: CommandBarProps) => {
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-8 h-8">
            <svg viewBox="0 0 32 32" className="w-full h-full">
              ...
              />
              <circle cx="8" cy="16" r="2" fill="hsl(var(--quantum-cyan))" className="quantum-pulse" />
              <circle cx="24" cy="16" r="2" fill="hsl(var(--quantum-magenta))" className="quantum-pulse" style={{ animationDelay: "1s" }} />
            </svg>
          </div>
...
const ModeButton = ({ active, onClick, children }: ModeButtonProps) => {
  return (
    ...
    >
      {children}
      {active && (
        <div className="absolute inset-0 rounded-md animate-quantum-glow pointer-events-none" />
      )}
    </button>
  );
};
