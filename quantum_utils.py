"""
Quantum utilities module for Q# integration
Clean, DRY implementation with automatic Q# reloading
"""
import qsharp
import os

class QuantumOperations:
    """Class to handle Q# quantum operations"""
    
    def __init__(self):
        """Initialize Q# environment and load operations"""
        qsharp.init()
        self.qs_code = None  # Store Q# code for reuse
    ...
    def _create_qubit_info_dict(self, qubit_obj):
        """Convert Python Qubit object to dictionary for Q# operations"""
        return {
            "id": qubit_obj.id,
    ...
    def _build_qubit_info_qs(self, qubit_info, var_name="qInfo"):
        """Build Q# QubitInfo variable declaration - DRY helper"""
    ...
    def _exec_qsharp(self, code):
        """Execute Q# code - reload definitions if NotFound error occurs"""
        try:
            return qsharp.eval(code)
        except Exception as e:
            error_str = str(e)
            # Only reload if we get a NotFound error
            if "NotFound" in error_str and self.qs_code:
                try:
                    qsharp.init()  # Reinitialize
                    qsharp.eval(self.qs_code)  # Reload definitions
                    return qsharp.eval(code)  # Retry
    ...
    def create_bell_state_simple(self):
        ...
        code = "QuantumEntanglement.CreateBellStates()"
        return self._exec_qsharp(code)
    
    def process_single_qubit(self, qubit_obj):
        """Process a single Python qubit using Q# operations"""
    ...
    def perform_teleportation(self, qubit1, qubit2):
        code = "QuantumEntanglement.Teleportation(qubit1, qubit2)"
        return self._exec_qsharp(code)
...
# Convenience functions - clean API
def create_bell_state():
    """Simple Bell state creation for demos"""
...
def entangle_qubits(qubit1, qubit2):
    """Create entanglement between two qubits with metadata"""
    return quantum_ops.process_two_qubits(qubit1, qubit2)

def perform_q_teleportation(qubit1, qubit2):
    return quantum_ops.perform_teleportation(qubit1, qubit2)
